...
Viewer::Viewer() : nanogui::Screen(Eigen::Vector2i(1024, 768), "DGP Viewer") {
    ...
    b->setCallback([this]() {
        mesh_->load_mesh("../data/scanned_face.off");
        this->refresh_mesh();
        this->refresh_trackball_center();
    });
    ...
    b = new Button(popup, "Max Nice Mesh");
    b->setCallback([this]() {
        mesh_->load_mesh("../data/nice_max.off");
        this->refresh_mesh();
        this->refresh_trackball_center();
    });
    ...
    b = new Button(popup, "Cotan Laplacian");
    b->setCallback([this]() {
        mesh_->smooth(10);
        mesh_->compute_mesh_properties();
        this->refresh_mesh();
    ...
    b = new Button(panel, "Cotan Laplacian");
    b->setCallback([this]() {
        mesh_->cotan_laplacian_enhance_feature(this->iterationTextBox->value(),
                                                this->coefTextBox->value());
        mesh_->compute_mesh_properties();
        this->refresh_mesh();
    });
    ...
    initShaders();
    mesh_ = new mesh_processing::MeshProcessing("../data/bunny.off", false);
    this->refresh_mesh();
    this->refresh_trackball_center();
}
